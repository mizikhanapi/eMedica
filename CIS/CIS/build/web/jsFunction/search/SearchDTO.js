  ...
  $(function () {
                $("#problem11").on('keyup', function () { // everytime keyup event
                    ...
                    if (input.length >= 1) { // Minimum characters = 2 (you can change)
                        $('#match12').html('<img src="img/LoaderIcon.gif" />'); // Loader icon apprears in the <div id="match"></div>
...
//js Search in update drug
 $(function () {
                $("#pproblem11").on('keyup', function () { // everytime keyup event
                    var input = $(this).val(); // We take the input value
                    if (input.length >= 1) { // Minimum characters = 2 (you can change)
                        $('#match13').html('<img src="img/LoaderIcon.gif" />'); // Loader icon apprears in the <div id="match"></div>
                        var dataFields = {'input': input}; // We pass input argument in Ajax
                        $.ajax({
                            type: "POST",
                            url: "search/Search2.jsp", // call the jsp file ajax/auto-autocomplete.php
