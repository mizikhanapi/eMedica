...
<tbody>
    ...
    <%
        String sql = " SELECT user_id, user_name, hfc_cd, hfc_name, mobile_phone, status FROM adm_users join adm_health_facility on health_facility_code = hfc_cd ";
    ...
    <tr>
        <input id="DT_hidden" type="hidden" value="<%=String.join("|", dataUser.get(i))%>">
        <td><%= dataUser.get(i).get(0)%></td>
        <td><%= dataUser.get(i).get(1)%></td>
        <td><%= dataUser.get(i).get(3)%></td>
        <td><%= dataUser.get(i).get(4)%></td>
        <td><%if(dataUser.get(i).get(5).equals("1"))
                out.print("Inactive"); 
              else
                out.print("Active"); %></td>
