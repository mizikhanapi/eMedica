<%-- 
    Document   : medicalStaffDoctorCheckAppointment
        ...
        {  
            if(dataCheckAppPastActive.size()>0) // check date for the inserted patient and check wheather the slot is active
            {
                %><script language='javascript'>
                    ...
                    window.location= 'medicalStaffDoctor.jsp';
                </script> <%
            }
            else
            {
                                ...
                                for(int index = 0; index < dataCheckApp.size(); index++)
                                {
                                    ...
                                    if(statusCanceled)
                                    {
                                        %><script language='javascript'>
                                            ...
                                            if(result == true)
                                            { 
                                                window.location= 'medicalStaffDoctorInsertAppointmentData.jsp?e27=<%=pmiNo%>&e30=<%=dataUserID%>&e31=<%=patientName%>&e32=<%=newAppDate%>&e33=<%=appDateTime%>&e34=<%=username%>&e35=<%=discipline%>&e36=<%=subdicipline%>&e46=<%=appType%>&e48=<%=dataHFCCode%>';
                                            }
                                            else
                                            {
                                                window.location= 'medicalStaffDoctor.jsp?e27=<%=pmiNo%>&e31=<%=patientName%>&e30=<%=idNo%>&e32=<%=appDate%>&e33=<%=appTime%>&e34=<%=appDoc%>&e35=<%=disApp%>&e36=<%=subDisApp%>';
