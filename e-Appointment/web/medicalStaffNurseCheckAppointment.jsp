<%-- 
    Document   : medicalStaffNurseCheckAppointment
...
//      QUERY VALIDATE LEAVE (staff not in leave - allowed (Doctor only)) date entered compare ngan tarikh end leave dan status yg active
        ...
        if(dataCheckExistDate.size()> 0)  //check date for everyone
        {
            %><script language='javascript'>
                ...
                window.location= 'medicalStaffDoctor.jsp?e27=<%=pmiNo%>&e31=<%=patientName%>&e30=<%=idNo%>&e32=<%=appDate%>&e33=<%=appTime%>&e34=<%=appDoc%>&e35=<%=disApp%>&e36=<%=subDisApp%>';
            </script> <%
        }
        else
        {  
            if(dataCheckAppPastActive.size()>0) // check date for the inserted patient and check wheather the slot is active
            {
                %><script language='javascript'>
                    ...
                    window.location= 'medicalStaffNurse.jsp';
                </script> <%
            }
            else
            {
                                ...
                                for(int index = 0; index < dataCheckApp.size(); index++)
                                {
                                    ...
                                    if(statusCanceled)
                                    {
                                        %><script language='javascript'>
                                            ...
                                            if(result == true)
                                            { 
                                                window.location= 'medicalStaffNurseInsertAppointmentData.jsp?e27=<%=pmiNo%>&e30=<%=dataUserID%>&e31=<%=patientName%>&e32=<%=newAppDate%>&e33=<%=appDateTime%>&e34=<%=username%>&e35=<%=discipline%>&e36=<%=subdicipline%>&e46=<%=appType%>&e48=<%=dataHFCCode%>';
                                            }
                                            else
                                            {
                                                window.location= 'medicalStaffNurse.jsp?e27=<%=pmiNo%>&e31=<%=patientName%>&e30=<%=idNo%>&e32=<%=appDate%>&e33=<%=appTime%>&e34=<%=appDoc%>&e35=<%=disApp%>&e36=<%=subDisApp%>';
                                            }
                                        </script> <% 
                                    }
